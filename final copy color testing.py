...
#using customtkinter automatically added scrollbars to textboxes hence why they're removed in this iteration
...
class App(ck.CTk): 
    ...
    def main_menu(self):
        ...
        frame_1 = ck.CTkFrame(self)
        frame_1.pack(pady=20, padx=60, fill="both", expand=True)

        button1 = ck.CTkButton(frame_1, width=30,text="WEBSITE SCANNING",command=self.show_websiteScan)
        button1.pack(fill=tk.BOTH,expand=True,pady=10)
        button2 = ck.CTkButton(frame_1, width=30,text="FILE SCANNING",command=self.show_fileScan)
        button2.pack(fill=tk.BOTH,expand=True,pady=10)
        button3 = ck.CTkButton(frame_1, width=30,text="SAFETY ADVICE", command=self.show_Advice)
        button3.pack(fill=tk.BOTH,expand=True,pady=10)
...
class fileScan(ck.CTkFrame): #inherts the root window (as we don't want new windows to pop up for each mode)
    ...
    def __init__(self,parent): #initialises instance, clears all existing widgets, sets up frames to prepare for new widgets
        ...
        frame = ck.CTkFrame(parent)
        frame.pack(fill=ck.BOTH,expand=True)
        myFrame = ck.CTkFrame(frame)
        myFrame.pack(side=ck.RIGHT,fill=tk.BOTH,expand=True)
        secFrame = ck.CTkFrame(frame)
        secFrame.pack(side=ck.TOP,pady=(15,0),padx=(10,0))
    ...
    def create_widgets(self,frame,myFrame,secFrame,parent): #sets up widgets and layout
        T = ck.CTkTextbox(frame,padx=10,pady=10,wrap=tk.WORD)
        scrollbar = ck.CTkScrollbar(frame, command=T.yview)
        mainMenu_button = ck.CTkButton(myFrame,width=30,text='MAIN MENU',command=lambda:toMainMenu(parent)) 
        quickScan_button = ck.CTkButton(myFrame,width=30,text='QUICK SCAN',command=lambda:self.quickScan(E,T))
        advancedScan_button = ck.CTkButton(myFrame,width=30,text='ADVANCED SCAN',command=lambda:self.advancedScan(E,T))
        E = ck.CTkEntry(secFrame,width=500)
        link_button = ck.CTkButton(secFrame,text='CHOOSE FILE',command=lambda:self.openFile(E))
        link_button.pack(side=ck.LEFT,padx=(12,20))
        E.pack(side=ck.LEFT,padx=(5,32),fill=ck.X,expand=True)
        mainMenu_button.pack(side=ck.BOTTOM,pady=10,padx=(5,20),fill=ck.X)
        quickScan_button.pack(side=ck.TOP,padx=(5,20),pady=(20,10),fill=ck.BOTH,expand=True)
        advancedScan_button.pack(side=ck.TOP,padx=(5,20),fill=ck.BOTH,expand=True)
        T.cget("font").configure(size=18)
        T.insert(ck.END, self.quickScanGuide)
        T.pack(padx=(20,20),pady=(10,10),fill=ck.BOTH,expand=True)
    ...
    def quickScan(self,E,T): #quick scan mode
        ...
        def scan(): #error prevention and making api calls to cloudmersive for quick scan
            ...
            if file_size_mb > 3:
                #print("ERORR ERROR ERROR")
                insertText(T,self.quickScanSizeError)
                messagebox.showinfo("SIZE ERROR", "FILE SIZE TOO BIG, SEE TEXTBOX FOR MORE INFO")
                return   
            try:
    ...
    def advancedScan(self,E,T): #advanced scan mode
        ...
        def scan(): 
            ...
            insertText(T,"UPLOADING FILE TO WEBSERVER FOR ANALYSIS... (MAY TAKE SOME TIME)")
            ...
            file_size_bytes = os.path.getsize(file_path)
            file_size_mb = file_size_bytes / (1024 * 1024)
            if file_size_mb < 125: #if file size is too big (over 125 MB)
                insertText(T,"\n\nTHERE WAS AN ERROR UPLOADING YOUR SELECTED FILE, PERHAPS THE SIZE OF YOUR FILE IS OVER 125MB. PLEASE MAKE USE OF A REPUTABLE ANTI-VIRUS SCANNER SUCH AS MCAFEE TO SCAN THIS FILE.") #in case file is too big
                messagebox.showinfo("SIZE ERROR", "FILE SIZE TOO BIG, SEE TEXTBOX FOR MORE INFO")
                return
            elif file_size_mb > 25: #needs different method of uploading file now due to api changes, so implementeed this to deal with this
                headers = {
                ...
                errr = json.loads(response.text)
                url = errr['data'] #defining url again to NEW url that next request should be posted to (as documented by Virustotal api)
            try:
                response = requests.post(url, files=files, headers=headers) #uploading file 
            except: #if file too big
                ...
                messagebox.showinfo("FILE ERROR", "PERHAPS FILE SIZE TOO BIG, SEE TEXTBOX FOR MORE INFO")
                return
            try:
                    grr = json.loads(response.text) #formatting response text into json for easy-to-read format
            except:
                    insertText(T,"\n\nERROR UPLOADING FILE, PERHAPS NETWORK SPEED IS TOO SLOW OR EXCEEDING YOUR NETWORK PROVIDER'S LIMITS?")
                    messagebox.showinfo("UPLOAD ERROR", "NETWORK-RELATED, SEE TEXTBOX FOR MORE INFO")
                    return
...
class websiteScan(ck.CTkFrame): #inherits the root window from App (so more windows don't pop up each time this class is called)
...
class showAdvice(ck.CTkFrame): #inherits root window to prevent additional windows popping up each time the class is called upon
    ...
    def __init__(self,parent): #clears existing widgets, sets up new layout and widgets
        ...
        frame = ck.CTkFrame(root)
        frame.pack(padx=10,fill=ck.BOTH,expand=True)
        myFrame = ck.CTkFrame(frame)
        myFrame.pack(side=ck.BOTTOM)
        self.create_widgets(frame,parent,myFrame)
    ...
    def create_widgets(self,frame,parent,myFrame):
        
        phishing_button = ck.CTkButton(myFrame, width=800,height=80, text='PHISHING', command=lambda: self.showPhishing(T)) #lambda used to prevent command from automatically executing (as T variable is passed to function which automatically executes code)
        malware_button = ck.CTkButton(myFrame, width=800, height=80,text='MALWARE', command=lambda: self.showMalware(T))
        mainMenu_button = ck.CTkButton(myFrame,width=800,height=80,text='MAIN MENU',command=lambda : toMainMenu(parent))
        mainMenu_button.pack(pady=(0,10),padx=20,side=ck.BOTTOM,fill=ck.BOTH,expand=True)
        malware_button.pack(padx=20,pady=5,side=ck.BOTTOM, fill=ck.BOTH, expand=True)
        phishing_button.pack(padx=20,side=ck.BOTTOM, fill=ck.BOTH, expand=True)
        T = ck.CTkTextbox(frame,padx=10,pady=10,wrap=ck.WORD)
        T.insert(ck.END, f"Select from either the 'PHISHING' or 'MALWARE' button below to display advice and information about the respective category.")
        T.pack(padx=(20),pady=(10,10),fill=ck.BOTH,expand=True)
        T.cget("font").configure(size=18)
